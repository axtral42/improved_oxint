...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("""Welcome to OXINT Bot! 🌐🔍
...
async def handle_image(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await send_local_image(update, context, file_path, url_var=False)  # Send the received image

async def send_local_image(update: Update, context: ContextTypes.DEFAULT_TYPE, file_path: str, url_var: bool,capt=""):
    chat_id = update.message.chat.id
    try:
        if url_var:
            await context.bot.send_photo(chat_id=chat_id, photo=file_path,caption=capt)
        else:
            await context.bot.send_photo(chat_id=chat_id, photo=open(file_path, 'rb'),caption=capt)
        print('Image sent successfully!')
    except Exception as e:
        print(f'Error sending image: {e}')

async def ox_name(name: str,keyword:str, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await send_message(update, context, message="Personal Profiles profiles:",img=False)
    for j in sm_profiles:
        for i in sm_profiles[j]:
            if j=="facebook":
    ...
    for j in sm_profiles:
        if j=="phonenum":
            for i in sm_profiles[j]:
                await send_message(update,context,"Phone Number Found, Starting Analysis")
            if i[0]=="0":
                i=i[1:]
...
async def send_message(update: Update, context: ContextTypes.DEFAULT_TYPE, message='', img=False, file_path='', url_var=False):
    chat_id = update.message.chat.id
    ...
    if not(img):
        ...
        await send_local_image(update,context,file_path,url_var,capt=f"Source: {file_path}")
